...
class DynamicConv2DDelta(nn.Module):
    ...
    through the TensorComposerDelta mechanism. The generated weights are projected onto a reshaped
    ...
    - Built-in repulsion, similarity penalty, and rank-weighted mixing via TensorComposerDelta
    ...
    def __init__(
        ...
        assert initialization_std > 0.0, "initialization_std should be > 0.0"
        assert weights_repulsion_strength >= 0.0, "weights_repulsion_strength should be >= 0.0"
        assert weights_noise_strength >= 0.0, "weights_noise_strength should be >= 0.0"
        assert rank_noise_strength >= 0.0, "rank_noise_strength should be >= 0.0"
        assert similarity_penalty_strength >= 0.0, "similarity_penalty_strength should be >= 0.0"
        ...
        self.wl_context_length = context_length
        self.wl_context_use_bias = context_use_bias
        self.wl_rank = rank
        self.wl_initialization_std = initialization_std
        self.wl_weights_repulsion_strength = weights_repulsion_strength
        self.wl_weights_noise_strength = weights_noise_strength
        self.wl_rank_noise_strength = rank_noise_strength
        self.wl_similarity_penalty_strength = similarity_penalty_strength
        ...
        # ================================================================================= #
        self.weights_lib = dyna.lib.TensorComposerDelta(
            output_shape=self.dynamic_weights_shape,
            context_length=self.wl_context_length,
            context_use_bias=self.wl_context_use_bias,
            rank=self.wl_rank,
            initialization_std=self.wl_initialization_std,
            weights_repulsion_strength=self.wl_weights_repulsion_strength,
            weights_noise_strength=self.wl_weights_noise_strength,
            rank_noise_strength=self.wl_rank_noise_strength,
            similarity_penalty_strength=self.wl_similarity_penalty_strength,
