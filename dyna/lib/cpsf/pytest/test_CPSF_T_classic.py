...
def test_T10_output_dtype_and_z_dtype_independence(N, dtype_z_pair, dtype_T):
    ...
    def _run(dtype_z):
        ...
        T_w = T_classic_window(
            z, z_j, vec_d, vec_dj, T_hat, alpha_base.to(REAL_T), sp, sq, offsets
        )
        T_f = T_classic_full(
...
def test_T11_permutation_invariance_over_j(device, N, dtype_z, dtype_T):
    ...
    T0 = T_classic_window(z, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets)
    T1 = T_classic_window(
        z, z_j_p, vec_d, vec_d_j_p, T_hat_j_p, alpha_j_p, sp_p, sq_p, offsets
    )
...
def test_T12_linearity(device, N, dtype_z, dtype_T):
    ...
    T_A_w = T_classic_window(
        z, z_j, vec_d, vec_d_j, T_hat_j_A, alpha_j_A, sp, sq, offsets
    )
    T_B_w = T_classic_window(
        z, z_j, vec_d, vec_d_j, T_hat_j_B, alpha_j_A, sp, sq, offsets
    )
    T_C_w = T_classic_window(
        z,
        z_j,
        vec_d,
        vec_d_j,
    ...
    T_b_w = T_classic_window(
        z, z_j, vec_d, vec_d_j, T_hat_j_A, alpha_j_B, sp, sq, offsets
    )
    T_c_w = T_classic_window(
        z,
        z_j,
...
def test_T13_cpu_gpu_isomorphism(N, dtype_z, dtype_T):
    ...
    T_cpu_w = T_classic_window(
        z, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets_cpu
    )
    T_gpu_w = T_classic_window(
        z.to(cuda),
        z_j.to(cuda),
        vec_d.to(cuda),
        vec_d_j.to(cuda),
...
def test_T14_boundary_periodicity_with_reindexed_offsets(device, N, dtype_z, dtype_T):
    ...
    T_L_w = T_classic_window(
        z_left, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets
    )
    T_R_w = T_classic_window(
        z_right, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets_shifted
    )
...
def test_T15_zero_radius_center_equals_central_image(device, N, dtype_z, dtype_T):
    ...
    T_w = T_classic_window(z, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, off0)
...
def test_T16_edge_forms_small_dimensions(device, N, dtype_z, dtype_T):
    ...
    T_w = T_classic_window(z, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets)
...
def test_T17_boundary_continuity_small_eps(device, N, dtype_z, dtype_T):
    ...
    def _D_eps(eps):
        ...
        T_w_L = T_classic_window(
            z_left, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets
        )
        T_w_R = T_classic_window(
            z_right_period, z_j, vec_d, vec_d_j, T_hat_j, alpha_j, sp, sq, offsets
        )
