...
@torch.no_grad()
def run_once(dev, dtype, B, N, M, S, n_chunk_a, n_chunk_b, m_chunk_a, m_chunk_b,
             quad_nodes, eps_total, scale_im, seed):
    R = real_dtype_of(dtype)
    ...
    sigma_perp_f  = torch.empty(M, device=dev, dtype=R).uniform_(0.4, 1.2, generator=g_sig)
    sigma_par_f   = torch.empty(M, device=dev, dtype=R).uniform_(1.0, 2.0, generator=g_sig)
    ...
    T_fused_A = T_PHC_Fused(
        z=z, vec_d=vec_d, z_j=z_j_f, vec_d_j=vec_d_j_f, T_hat_j=T_hat_j_f,
        alpha_j=alpha_j_f, sigma_par_j=sigma_par_f, sigma_perp_j=sigma_perp_f,
        quad_nodes=quad_nodes, eps_total=eps_total,
        n_chunk=n_chunk_a, m_chunk=m_chunk_a, dtype_override=dtype,
    )
    T_fused_B = T_PHC_Fused(
        z=z, vec_d=vec_d, z_j=z_j_f, vec_d_j=vec_d_j_f, T_hat_j=T_hat_j_f,
        alpha_j=alpha_j_f, sigma_par_j=sigma_par_f, sigma_perp_j=sigma_perp_f,
        quad_nodes=quad_nodes, eps_total=eps_total,
        n_chunk=n_chunk_a, m_chunk=m_chunk_b, dtype_override=dtype,
    )
    ...
    T_fused_C = T_PHC_Fused(
        ...
        alpha_j=alpha_j_f, sigma_par_j=sigma_par_f, sigma_perp_j=sigma_perp_f,
        quad_nodes=quad_nodes, eps_total=eps_total,
        n_chunk=n_chunk_b, m_chunk=m_chunk_a, dtype_override=dtype,
    )
    ...
    T_batched_A = T_PHC_Batched(
        z=z, vec_d=vec_d, z_j=z_j_b, vec_d_j=vec_d_j_b, T_hat_j=T_hat_j_b,
        alpha_j=alpha_j_b, sigma_par_j=sigma_par_b, sigma_perp_j=sigma_perp_b,
        quad_nodes=quad_nodes, eps_total=eps_total,
        n_chunk=n_chunk_a, m_chunk=m_chunk_a, dtype_override=dtype,
    )
...
def main():
    ...
    print(f"Device={dev.type}, dtype={dtype}, B={args.B}, N={args.N}, M={args.M}, S={args.S}")
    ...
    run_once(dev, dtype, args.B, args.N, args.M, args.S,
             n_chunk_a, n_chunk_b, m_chunk_a, m_chunk_b,
             args.quad_nodes, args.eps_total, scale_im=0.0, seed=args.seed)

    run_once(dev, dtype, args.B, args.N, args.M, args.S,
             n_chunk_a, n_chunk_b, m_chunk_a, m_chunk_b,
             args.quad_nodes, args.eps_total, scale_im=1.0, seed=args.seed+1000)
