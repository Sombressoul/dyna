# dyna/lib/cpsf/benchmark/benchmark_CPSF_T_PHC_Batched.py
# Run examples:
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_T_PHC_Batched --N 256 --M 256 --S 128 --batch 128 --dtype c64 --device cuda --iters 50 --warmup 10 --n_chunk 256 --m_chunk 256 --quad_nodes 7
...
def _make_unit_batch_3d(
    ...
    g = torch.Generator(device=device).manual_seed(seed)
    REAL = _real_dtype_of(dtype)
    xr = torch.randn(B, M, N, generator=g, device=device, dtype=REAL)
    xi = torch.randn(B, M, N, generator=g, device=device, dtype=REAL)
    v = (xr + 1j * xi).to(dtype)
    n = torch.linalg.vector_norm(v, dim=-1, keepdim=True)
    n = torch.where(n.real == 0, torch.ones_like(n), n)
    return v / n
...
def _make_complex(
    shape: tuple[int, ...], dtype: torch.dtype, device: torch.device, seed: int
):
    g = torch.Generator(device=device).manual_seed(seed)
    REAL = _real_dtype_of(dtype)
...
def main():
    ...
    ap.add_argument(
        "--M", type=int, default=256, help="Number of contributions per query"
    )
    ap.add_argument(
    ...
    vec_d = _make_unit_batch_2d(B, N, CDTYPE, dev, seed=args.seed + 2)

    # Per-query pools (B, M, ...)
    z_j = _make_complex((B, M, N), CDTYPE, dev, seed=args.seed + 3)
    vec_d_j = _make_unit_batch_3d(B, M, N, CDTYPE, dev, seed=args.seed + 4)
    ...
    # mildly anisotropic, positive, with gap sp > sq   (shapes: [B, M])
