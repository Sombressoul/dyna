# Run as (example):
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_R --N 16 --batch 65536 --dtype c64 --device cuda --iters 100 --warmup 10
# For full stats:
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_R --N 16 --batch 65536 --dtype c64 --device cuda --iters 100 --warmup 10 --profile --verify_devices

import argparse, time, torch
from ..functional.core_math import R

def _fmt_bytes(x:int)->str:
    u=["B","KB","MB","GB","TB"]; i=0; v=float(x)
...
def main():
    ...
    # Optional: fix default device so all *new* tensors without explicit device land on dev
    try:
    ...
    print(f"Device={dev.type}, dtype={dtype}, batch={B}, N={N}, warmup={args.warmup}, iters={args.iters}")

    d = make_batch(B,N,dtype,dev)

    if args.verify_devices:
        out = R(d)
        print(f"in.device={d.device}, out.device={out.device}")
        assert (dev.type=="cpu" and (not d.is_cuda) and (not out.is_cuda)) or \
               (dev.type=="cuda" and d.is_cuda and out.is_cuda), "Device mismatch!"
        del out
    ...
    if args.profile:
        ...
        with profile(activities=activities, schedule=sch, record_shapes=False, profile_memory=True) as prof:
            for _ in range(5):
                _ = R(d)
                prof.step()
        print(prof.key_averages().table(sort_by="self_cuda_time_total" if dev.type=="cuda" else "cpu_time_total", row_limit=15))
    ...
    for _ in range(args.iters):
        if dev.type=="cuda":
            ...
            start.record()
            out = R(d)
            # keep the work; avoid full DCE; .item() will sync by design (fine for timing)
    ...
    print(f"Throughput:    {thr:,.0f} vec/s")
    if dev.type=="cuda":
