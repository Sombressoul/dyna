# Run as (examples):
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_T_classic_window --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cpu --iters 50 --warmup 10 --window 3
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_T_classic_window --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cuda --iters 50 --warmup 10 --window 3 --profile
#
# Notes:
# - Includes overhead of constructing CPSFPeriodization(WINDOW, window=W) and materializing the single offsets batch.
# - Calling contract mirrors tests: T_classic_window(..., offsets_iterator=per.iter_offsets).  # see tests

# Results (reference):
#
# CPU (Ryzen 9 - 5950x):
#   > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_T_classic_window --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cpu --iters 50 --warmup 10 --window 3
#       Device=cpu, dtype=torch.complex64, N=4, M=256, S=64, B=32, W=3, warmup=10, iters=50, sigma_par=1.25, sigma_perp=0.55, q_max=None, cache_active=False
#
#       === T_classic_window Benchmark ===
#       Avg time/iter: 1498.987 ms  (± 100.360 ms)
#       Throughput:    21 fields/s  (each has S=64)
#       Offsets total (|n|_inf<=W): ~2,401 points (theoretical (2W+1)^N)
#       Process RSS:   1.84 GB
#
# GPU (RTX 4090):
#   > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_T_classic_window --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cuda --iters 50 --warmup 10 --window 3
#       Device=cuda, dtype=torch.complex64, N=4, M=256, S=64, B=32, W=3, warmup=10, iters=50, sigma_par=1.25, sigma_perp=0.55, q_max=None, cache_active=False
#
#       === T_classic_window Benchmark ===
#       Avg time/iter: 77.042 ms  (± 15.161 ms)
#       Throughput:    415 fields/s  (each has S=64)
#       Offsets total (|n|_inf<=W): ~2,401 points (theoretical (2W+1)^N)
#       CUDA peak mem (max):   5.58 GB
#       CUDA alloc Δ (max):    16.00 KB


import argparse, time, torch
from ..functional.core_math import T_classic_window
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=3,
        help="WINDOW radius W (one-shot cube |n|_inf<=W)",
    )
    ...
    if args.window is None or args.window < 1:
        raise SystemExit("--window must be integer >= 1")

    print(
        f"Device={dev.type}, dtype={CDTYPE}, N={N}, M={M}, S={S}, B={B}, "
        f"W={args.window}, warmup={args.warmup}, iters={args.iters}, "
        f"sigma_par={args.sigma_par}, sigma_perp={args.sigma_perp}, "
        f"q_max={args.q_max}, cache_active={'True' if args.cache_active else 'False'}"
    )
    ...
    if args.verify_devices:
        per0 = CPSFPeriodization(
            kind=CPSFPeriodizationKind.WINDOW,
    ...
    print(
        f"Offsets total (|n|_inf<=W): ~{offset_count:,} points (theoretical (2W+1)^N)"
    )
    if dev.type == "cuda":
