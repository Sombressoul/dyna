# Run as (examples):
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_Tau_nearest --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cpu --iters 50 --warmup 10
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_Tau_nearest --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cuda --iters 50 --warmup 10 --profile
#
# Notes:
# - Measures Tau_nearest end-to-end (no periodization / no k).
# - Shapes aligned exactly as required by delta_vec_d: we expand to (B, M, N).

# Results (reference):
#
# CPU (Ryzen 9 - 5950x):
#   > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_Tau_nearest --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cpu --iters 50 --warmup 10
#       Device=cpu, dtype=torch.complex64, N=4, M=256, S=64, B=32, warmup=10, iters=50, sigma_par=1.25, sigma_perp=0.55, q_max=None
#
#       === Tau_nearest Benchmark ===
#       Avg time/iter: 31.910 ms  (± 0.136 ms)
#       Throughput:    1,003 fields/s  (each has S=64)
#       Process RSS:   457.31 MB
#
# GPU (RTX 4090):
#   > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_Tau_nearest --N 4 --M 256 --S 64 --batch 32 --dtype c64 --device cuda --iters 50 --warmup 10
#       Device=cuda, dtype=torch.complex64, N=4, M=256, S=64, B=32, warmup=10, iters=50, sigma_par=1.25, sigma_perp=0.55, q_max=None
#
#       === Tau_nearest Benchmark ===
#       Avg time/iter: 4.593 ms  (± 0.384 ms)
#       Throughput:    6,968 fields/s  (each has S=64)
#       CUDA peak mem (max):   88.91 MB
#       CUDA alloc Δ (max):    16.00 KB

import argparse, time, torch
from ..functional.core_math import Tau_nearest
...
def main():
    ...
    print(
        ...
        f"sigma_par={args.sigma_par}, sigma_perp={args.sigma_perp}, q_max={args.q_max}"
    )
