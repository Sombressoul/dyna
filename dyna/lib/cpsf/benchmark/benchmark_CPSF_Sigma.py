# Run as (example):
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_Sigma --N 16 --batch 65536 --dtype c64 --device cuda --iters 100 --warmup 10
# For full stats:
# > python -m dyna.lib.cpsf.benchmark.benchmark_CPSF_Sigma --N 16 --batch 65536 --dtype c64 --device cuda --iters 100 --warmup 10 --profile --verify_devices

import argparse, time, torch
from ..functional.core_math import R, R_ext, Sigma
...
def main():
    ...
    # Prepare inputs: vectors -> base frames -> extended frames (outside timed loop)
    d = make_batch(B, N, dtype, dev)      # (B, N)
    R_b = R(d)                            # (B, N, N)
    Rext_b = R_ext(R_b)                   # (B, 2N, 2N)
    del d, R_b

    # Fixed positive anisotropic sigmas
    sp = torch.tensor(1.25, dtype=REAL, device=dev)
    sq = torch.tensor(0.55, dtype=REAL, device=dev)

    if args.verify_devices:
        out = Sigma(Rext_b, sp, sq)
        print(f"in.device={Rext_b.device}, out.device={out.device}")
        assert (
            (dev.type == "cpu" and (not Rext_b.is_cuda) and (not out.is_cuda))
            or (dev.type == "cuda" and Rext_b.is_cuda and out.is_cuda)
        ), "Device mismatch!"
        del out
    ...
    for _ in range(args.iters):
        if dev.type == "cuda":
            ...
            start.record()
            out = Sigma(Rext_b, sp, sq)  # (B, 2N, 2N)
    ...
    thr = B / (avg / 1e3)  # covariances/s (each item is one Σ(R_ext, sp, sq))

    print("\n=== Sigma(R_ext, sp, sq) Benchmark ===")
    print(f"Avg time/iter: {avg:.3f} ms  (± {std:.3f} ms)")
    print(f"Throughput:    {thr:,.0f} cov/s")
    if dev.type == "cuda":
