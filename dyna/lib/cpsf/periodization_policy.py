...
class CPSFPeriodizationPolicy:
    def __init__(
        ...
        # Enum guards
        if not isinstance(kind, CPSFPeriodizationPolicyKind):
            raise TypeError(
                f"CPSFPeriodizationPolicy: 'kind' must be CPSFPeriodizationPolicyKind, got {type(kind)}"
            )
        if not isinstance(backend, CPSFPeriodizationPolicyBackend):
            raise TypeError(
        ...
        # Per-kind validation
        if kind == CPSFPeriodizationPolicyKind.WINDOW:
            ...
            if backend != CPSFPeriodizationPolicyBackend.AUTO:
                raise ValueError(
                    "CPSFPeriodizationPolicy(WINDOW): 'backend' must be AUTO"
                )
        elif kind == CPSFPeriodizationPolicyKind.FULL:
