...
$$
\boxed{\;\mathbb E[\,S(x,y)\,]\;=\;Z(x,y).\;}\tag{2.2}
$$
...
$$
\mathbb E[S]=\sum_{j=0}^{d'-1}\bigl(d'^{-K}\prod_{k}\langle x_k,y_k\rangle\bigr)=Z(x,y),\tag{2.8}
$$

because the outer sum cancels the factor \$d'^{-K}\$.  This completes the proof. *∎*

---

### 2.5 What Is (and Isn’t) Unbiased

Equation (2.2) shows that **kernels** built from CSP sketches are unbiased.  The feature vector \$\Phi(x)\$ itself need *not* satisfy \$\mathbb E\[\Phi(x)]=\text{exact feature}\$ unless \$K=1\$.  Consequently, analyses that require unbiased feature gradients (e.g., certain SGD noise arguments) must be phrased directly in terms of the kernel.
